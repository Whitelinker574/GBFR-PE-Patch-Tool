...
test('all async page actions share one epoch-aware operation gate', () => {
  ...
  for (const operation of ['connect', 'disconnect', 'capture-enable', 'capture-disable', 'capture-refresh', 'item-read', 'spatial-read', 'spatial-teleport', 'flight-change']) {
...
test('spatial diagnostics use the verified three-snapshot reader and isolate the experimental writer', () => {
  ...
  assert.match(source, /class="monitor-empty coordinate-empty ui-empty"/)
  assert.match(source, /\.coordinate-empty\s*\{[^}]*min-height:72px[^}]*margin-top:0/s)
})

test('continuous jump is an independent optional feature beside the recommended virtual ground flow', () => {
  assert.match(source, /RuntimeSpatialJumpStatusOwned\(acquiredOwnerToken\)/)
  assert.match(source, /RuntimeSpatialJumpSetEnabledOwned\(ownerToken, enabled\)/)
  assert.match(source, /normalizeRuntimeSpatialJumpStatus/)
  ...
  assert.match(source, /class="flight-feature continuous-jump-feature ui-card is-flat"/)
  assert.match(source, /t\('spatialContinuousJumpTitle'\)/)
  assert.match(source, /t\('spatialContinuousJumpSummary'\)/)
  assert.ok(source.indexOf('continuous-jump-feature') < source.indexOf('coordinate-tools'), 'continuous jump must remain visible before exact-coordinate tools')
...
test('advanced coordinate diagnostics retain their bounded manual nudge controls', () => {
  assert.match(source, /const FLIGHT_FRAME_MS = 45/)
  assert.match(source, /const coordinateNudgeSpeed = ref\(8\)/)
  assert.match(source, /speed \* FLIGHT_FRAME_MS \/ 1000/)
  assert.match(source, /speed < 0\.1 \|\| speed > 1000/)
  assert.match(source, /v-model\.number="coordinateNudgeSpeed"/)
  assert.match(source, /min="0\.1" max="1000" step="0\.1"/)
  assert.doesNotMatch(source, /const flightStep|v-model="flightStep"/)
})

test('spatial movement makes virtual ground primary and keeps aerial actions as a collapsed experiment', () => {
  ...
  assert.match(source, /RuntimeSpatialHotkeysSetFlightModeOwned\(ownerToken, enabled, flightMode\.value\)/)
  assert.match(source, /const flightMode = ref\('virtual_ground'\)/)
  assert.match(source, /async function enableVirtualGround\(\)[\s\S]*?flightMode\.value = 'virtual_ground'[\s\S]*?setTrueFlightEnabled\(true\)/)
  assert.match(source, /async function enableAerialExperiment\(\)[\s\S]*?flightMode\.value = 'aerial'[\s\S]*?setTrueFlightEnabled\(true\)/)
  assert.match(source, /class="flight-lab virtual-ground-deck ui-card is-flat"/)
  assert.match(source, /@click="enableVirtualGround"/)
  assert.doesNotMatch(source, /class="flight-mode-switch ui-seg"/)
  assert.match(source, /class="flight-feature aerial-experiment ui-card is-flat"/)
  assert.match(source, /t\('spatialAerialExperimentBoundary'\)/)
  assert.ok(source.indexOf('virtual-ground-deck') < source.indexOf('coordinate-tools'), 'virtual ground must be the first spatial workflow')
  ...
  assert.match(source, /setTrueFlightEnabled\(false\)/)
  assert.match(source, /v-model\.number="flightSpeed"[^>]*min="0\.1" max="20"/)
  assert.match(source, /<kbd>W A S D<\/kbd>/)
  assert.match(source, /<kbd>PageUp<br \/>PageDown<\/kbd>/)
  assert.match(source, /t\('spatialMovementAxes'\)/)
  assert.match(source, /class="flight-diagnostics"/)
  assert.match(source, /contactTemplateReady/)
  assert.match(source, /acceptedContacts/)
...
test('the page keeps the parchment atom system responsive from narrow to ultra-wide containers', () => {
  ...
  assert.match(source, /@container runtime-monitor \(max-width:920px\)[\s\S]*?\.virtual-ground-layout/)
  assert.match(source, /@container runtime-monitor \(max-width:720px\)/)
  assert.match(source, /@container runtime-monitor \(max-width:720px\)[\s\S]*?\.monitor-connection\s*\{[^}]*flex-flow:row wrap/s)
  assert.match(source, /@container runtime-monitor \(max-width:720px\)[\s\S]*?\.connection-actions\s*\{[^}]*grid-template-columns:repeat\(2/s)
  assert.match(source, /@container runtime-monitor \(max-width:460px\)[\s\S]*?\.movement-key-grid/)
