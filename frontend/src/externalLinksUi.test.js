...
test('release metadata uses v1.91.10 consistently', () => {
  assert.match(readRoot('internal/backend/app.go'), /appVersion\s*=\s*"v1\.91\.10"/)
  assert.equal(JSON.parse(readRoot('frontend/package.json')).version, '1.91.10')
  assert.equal(JSON.parse(readRoot('frontend/package-lock.json')).version, '1.91.10')
  assert.equal(JSON.parse(readRoot('wails.json')).info.productVersion, '1.91.10')
})
