...
test('offline summon writes encodable values by default and keeps natural rules as small hints', () => {
  ...
  assert.match(source, /固定词条与等级已校验/)
  ...
  assert.doesNotMatch(source, /:disabled="mode === 'update'"/)
  assert.match(source, /更换种类会以新 SlotID 原子重建该实例/)
  assert.match(source, /已装备的四槽引用会一并迁移/)
