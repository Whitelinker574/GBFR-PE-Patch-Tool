...
const legacyUnknownBuildBoundary = /^(.*?)仅支持已识别的游戏 2\.0\.2 \/ 2\.0\.3(?: \/ 2\.0\.4)?(?: \/ 2\.0\.5)? 可执行文件；当前游戏版本不会连接或写入$/u
...
export function translateRuntimeCompatibilityError(value, locale = 'zh') {
  ...
  if (coded?.[1] === 'GBFR_RUNTIME_UNKNOWN_EXE' || match) {
    return `${featureLabel(match?.[1])} supports the recognized game 2.0.2, 2.0.3, 2.0.4, and 2.0.5 executables. This unknown game build will not be connected to or modified.`
  }
  return display
}
