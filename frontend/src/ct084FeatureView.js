...
export function validateCT084StatusSet(features, statuses) {
  ...
  for (const status of statuses) {
    ...
    if (typeof status?.enabled !== 'boolean') {
      throw new TypeError(`CT 0.8.4 回读状态 ${statusID} 的 enabled 必须是布尔值`)
    }
    if (typeof status.available !== 'boolean') {
      throw new TypeError(`CT 0.8.4 回读状态 ${statusID} 的 available 必须是布尔值`)
    }
    if (typeof status.error !== 'string') {
      throw new TypeError(`CT 0.8.4 回读状态 ${statusID} 的 error 必须是字符串`)
    }
    if (!Array.isArray(status.rvas)) {
      throw new TypeError(`CT 0.8.4 回读状态 ${statusID} 的 rvas 必须是数组`)
    }
    if (!Array.isArray(status.currentBytes)) {
      throw new TypeError(`CT 0.8.4 回读状态 ${statusID} 的 currentBytes 必须是数组`)
    }
    if (status.rvas.length !== status.currentBytes.length) {
      throw new Error(`CT 0.8.4 回读状态 ${statusID} 的 rvas 与 currentBytes 长度必须一致`)
    }
    ...
    if (status.rvas.length > 0 && status.rvas.length !== sites.length) {
      throw new Error(`CT 0.8.4 回读状态 ${statusID} 的写入点数量与目录不一致`)
    }
    status.rvas.forEach((rva, index) => {
      if (!Number.isSafeInteger(rva) || rva < 0) {
        throw new TypeError(`CT 0.8.4 回读状态 ${statusID} 的 RVA[${index}] 必须是非负安全整数`)
      }
    })
    status.currentBytes.forEach((currentBytes, index) => {
      if (typeof currentBytes !== 'string' || (currentBytes !== '' && !/^(?:[0-9a-f]{2})(?: [0-9a-f]{2})*$/iu.test(currentBytes))) {
        throw new TypeError(`CT 0.8.4 回读状态 ${statusID} 的当前字节[${index}] 必须是空值或空格分隔的十六进制字节`)
      }
      const enableBytes = catalogPatchBytes(sites[index]?.enableBytes)
      if (!enableBytes) {
        throw new Error(`CT 0.8.4 功能目录 ${statusID} 的补丁字节无效`)
      }
      if (currentBytes && currentBytes.split(' ').length !== enableBytes.length) {
        throw new Error(`CT 0.8.4 回读状态 ${statusID} 的当前字节[${index}] 长度与目录补丁不一致`)
      }
    })

    if (status.enabled && status.rvas.length === 0) {
      throw new Error(`CT 0.8.4 回读状态 ${statusID} 已开启却没有持有写入点`)
    }
    if (status.enabled && !status.available) {
      throw new Error(`CT 0.8.4 回读状态 ${statusID} 已开启时 available 必须为 true`)
    }
    if (status.enabled && status.error !== '') {
      throw new Error(`CT 0.8.4 回读状态 ${statusID} 已开启时 error 必须为空`)
    }
    if (status.enabled) {
      status.currentBytes.forEach((currentBytes, index) => {
        ...
        if (currentBytes.toLocaleLowerCase() !== expected) {
          throw new Error(`CT 0.8.4 回读状态 ${statusID} 已开启，但当前字节[${index}] 与目录补丁不一致`)
        }
      })
    }
  }
