...
test('monster safety copy is a full-width notice and raw patch bytes are disclosed on demand', () => {
  ...
  assert.match(source, /monster_damage_new/, 'party-wide damage entry must be the visible multiplier')
	assert.match(source, /item\.id === 'od_rate'/, 'OD gauge rate must have its own multiplier control')
	assert.match(source, /needsOverdriveState\(item\)/, 'forced OD state must remain a separate selector')
	assert.match(source, /OD 槽积累与减少速度 2 倍/, 'OD gauge rate needs an explicit user-facing multiplier example')
