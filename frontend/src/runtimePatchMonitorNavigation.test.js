...
test('party detection, spatial controls, and selected-item diagnostics share one persistent implementation but expose separate destinations', () => {
  ...
  assert.match(shell, /spatialTools:\s*\{\s*group:\s*['"]runtimeTools['"]/)
  assert.match(shell, /selectedItemMonitor:\s*\{\s*group:\s*['"]tools['"]/)
  assert.match(shell, /formulaSampler:\s*\{\s*group:\s*['"]tools['"]/)
  assert.match(shell, /id:\s*['"]loadoutFlow['"][\s\S]*?items:\s*\[['"]runtimeMonitor['"]/)
...
test('loadout detection is a persistent background service with local quest history', () => {
  ...
  assert.match(shell, /runtimeMonitor:\s*'background'/)
  assert.match(shell, /function toolTabTitle\(id\)[\s\S]*?Starts only when you click; then keeps running in the background[\s\S]*?点击后才开启，并持续在后台运行/)
  assert.match(shell, /function toolTagLabel\(id\)[\s\S]*?background:\s*'Background'[\s\S]*?background:\s*'后台'/)
  assert.match(shell, /:title="toolTabTitle\(id\)"/)
  assert.match(shell, /:class="\['switcher-tag', toolNavigationMode\(id\)\]">\{\{ toolTagLabel\(id\) \}\}/)
})
