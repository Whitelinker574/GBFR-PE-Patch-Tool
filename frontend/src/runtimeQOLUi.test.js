...
test('convenience runtime has one themed persistent workspace', () => {
  ...
  assert.match(shell, /items: \['runtime', 'runtimeQOL'/)
  ...
  assert.match(lab, /sessionCapture/)
	...
	assert.match(lab, /normalQuestLevelSync" type="checkbox" disabled/)
	assert.match(lab, /returnWrightstone" type="checkbox" disabled/)
	assert.match(lab, /本构建不会安装该 Hook/)
