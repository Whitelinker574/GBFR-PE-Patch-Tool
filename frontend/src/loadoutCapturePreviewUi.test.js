...
test('Logs library header keeps compact actions and a readable responsive hierarchy', () => {
  assert.match(viewer, /class="subpage-bar logs-library-bar ui-card"/)
  assert.match(viewer, /class="back-button logs-back-button ui-btn is-ghost"/)
  assert.match(viewer, /class="logs-source-button ui-btn is-primary"/)
  assert.match(viewer, /\.logs-library-bar\s*\{[^}]*grid-template-areas:\s*"back back"\s*"copy action"/is)
  assert.match(viewer, /\.logs-back-button\s*\{[^}]*justify-self:\s*start[^}]*width:\s*auto/is)
