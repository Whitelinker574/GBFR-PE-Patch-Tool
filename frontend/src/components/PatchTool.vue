...
button:focus-visible,input:focus-visible,select:focus-visible { outline:2px solid rgba(154,116,64,.55); outline-offset:2px; }
...
/* 搜索框输入允许在 flex 内收缩(min-width:0)、box-sizing:border-box，小窗下右边框不再被切断 */
.tool-panel :deep(.picker-search input) { box-sizing:border-box!important; min-width:0!important; width:100%!important; flex:1 1 auto!important; border:1px solid rgba(154,116,64,.3)!important; border-radius:4px!important; background:#fffdf6!important; }
...
/* Unify the 即时因子编辑 picker with the 因子/祝福 catalog dropdown: identical
   trigger/value weight, and dimmed placeholder (the picker placeholder was
   wrongly forced to font-weight:800 by an earlier :deep rule, making it look
   bold next to the catalog's normal-weight placeholder). Last in source order
   so it wins the !important cascade. */
.tool-panel :deep(.picker-selected),
.tool-panel :deep(.picker-label),
.tool-panel :deep(.catalog-trigger),
.tool-panel :deep(.catalog-value) { font-weight:560!important; }
.tool-panel :deep(.picker-placeholder),
.tool-panel :deep(.catalog-placeholder) { font-weight:560!important; color:#9c8a73!important; }
</style>
