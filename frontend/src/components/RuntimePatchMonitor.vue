      ...
      <section class="flight-lab ui-card is-flat">
        ...
        <div class="flight-console">
          ...
          <div class="flight-settings">
            ...
            <div class="flight-capability" :class="{ 'is-active': gravityStatus?.enabled, 'has-error': gravityError || gravityStatus?.error }"><span><b>{{ t('spatialGravity') }}</b><small>{{ gravityDetail }}</small></span><button v-if="gravityStatus?.enabled || gravityStatus?.recoveryPending" type="button" class="ui-btn is-sm is-ghost" :disabled="interactionLocked || !connected" @click="setGravityEnabled(false)">{{ gravityChanging ? t('spatialGravityChanging') : t('spatialGravityDisable') }}</button><button v-else type="button" class="ui-btn is-sm" disabled>{{ t('spatialUnavailable') }}</button></div>
