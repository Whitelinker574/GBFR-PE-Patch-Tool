...
import {
  CharaAcquire,
  CharaRelease,
  CT084PartyMonitorOwned,
  CT084SelectedItemReadOwned,
  CT084SelectedItemsDisableOwned,
  CT084SelectedItemsEnableOwned,
  CT084SelectedItemsStatusOwned,
...
import {
  consumeCT084SelectedCapture,
  formatRuntimeAddress,
  formatRuntimeCoordinate,
  formatRuntimeInteger,
  normalizeCT084PartySnapshot,
  normalizeCT084SelectedRecord,
  normalizeCT084SelectedStatus,
  partyOptionalMetric,
  runtimeMonitorRoleName,
  runtimeMonitorText,
  selectedCapturePhase,
} from '../ct084RuntimeMonitorView.js'

const emit = defineEmits(['status'])
const RUNTIME_LEASE_SCOPE = 'ct084-runtime-monitor'
...
async function refreshParty() {
  ...
  try {
    ...
    const snapshot = normalizeCT084PartySnapshot(
      await CT084PartyMonitorOwned(ownerToken),
      ownerToken,
...
async function enableCapture() {
  ...
  try {
    if (!ownerToken || !connected.value) throw new Error(t('statusConnect'))
    const status = normalizeCT084SelectedStatus(
      await CT084SelectedItemsEnableOwned(ownerToken),
      ownerToken,
      processInfo.value.pid,
    )
    if (!operationIsCurrent(operationToken, epoch, ownerToken)) return
...
async function disableCapture() {
  ...
  try {
    if (!ownerToken || !connected.value) throw new Error(t('statusConnect'))
    const status = normalizeCT084SelectedStatus(
      await CT084SelectedItemsDisableOwned(ownerToken),
      ownerToken,
      processInfo.value.pid,
    )
    if (!operationIsCurrent(operationToken, epoch, ownerToken)) return
...
async function refreshCaptureStatus() {
  ...
  try {
    if (!ownerToken || !connected.value) throw new Error(t('statusConnect'))
    const status = normalizeCT084SelectedStatus(
      await CT084SelectedItemsStatusOwned(ownerToken),
      ownerToken,
      processInfo.value.pid,
    )
    if (!operationIsCurrent(operationToken, epoch, ownerToken)) return
...
async function readSelectedItem(kind) {
  ...
  try {
    ...
    const record = normalizeCT084SelectedRecord(
      await CT084SelectedItemReadOwned(ownerToken, { kind, expectedSelectedAddr }),
      kind,
    ...
    selectedStatus.value = consumeCT084SelectedCapture(selectedStatus.value, kind)
...
<template>
  <section class="ct084-monitor-page ui-page ui-page-stack is-fluid" data-page="ct084-runtime-monitor" :aria-busy="operationBusy">
...
@container runtime-monitor (max-width:480px) {
  .ct084-monitor-page { gap:var(--space-3); padding-bottom:var(--space-5); }
