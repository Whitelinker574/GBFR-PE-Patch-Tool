  ...
  <div class="virtual-lab ui-page-stack">
    ...
    <section class="source-setup" :aria-label="tx('第一步 · 选择因子来源', 'Step 1 · Choose the Sigil Source')">
      <SaveSourcePicker
        ...
        @browse="chooseSave"
      />
      <label class="slot-count"><span>{{ tx('每名角色的额外槽数', 'Extra Slots per Character') }}</span><input v-model.number="slotCount" class="ui-input" type="number" min="1" max="8" step="1" /><small>{{ tx('可选 1–8；不是存档物理槽位', 'Choose 1–8; these are not physical save slots') }}</small></label>
...
@container virtual-lab (max-width:720px) { .virtual-intro,.source-setup { grid-template-columns:minmax(0,1fr); }.slot-count { grid-column:auto; }.slot-panel > header,.inventory-panel > header,.trait-ledger > header,.preset-panel > header,.virtual-dock { align-items:stretch; flex-direction:column; }.source-mode-tabs,.preset-panel > header > div:last-child,.dock-actions,.dock-actions .ui-btn { width:100%; }.trait-grid,.preset-list { grid-template-columns:minmax(0,1fr); } }
@container virtual-lab (max-width:500px) { .inventory-list,.source-constructor { grid-template-columns:minmax(0,1fr); }.source-constructor > .ui-notice,.constructor-wide,.constructor-submit { grid-column:auto; }.preset-panel > header > div:last-child { flex-direction:column; }.virtual-slots article { grid-template-columns:28px 34px minmax(0,1fr) 28px; }.trait-grid dl { gap:var(--space-2); } }
</style>
