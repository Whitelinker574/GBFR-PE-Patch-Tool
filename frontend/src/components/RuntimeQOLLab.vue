...
<template>
  <div class="qol-lab ui-page-stack">
    <section class="qol-intro">
      <div><p>INTEGRATED QOL RUNTIME · DLC 2.0.2</p><h2>{{ tx('游戏便利运行时', 'Game Convenience Runtime') }}</h2><span>{{ tx('把显示、联机信息、任务同步和编队便利集中到一个可恢复的内置运行时；无需外部加载器。', 'Keep display, session, quest-sync, and formation conveniences in one restorable built-in runtime without an external loader.') }}</span></div>
    ...
    <section class="qol-options ui-card ui-panel">
      ...
      <div class="option-grid">
        ...
        <label class="qol-option is-candidate"><input v-model="config.normalQuestLevelSync" type="checkbox" disabled /><span><b>{{ tx('普通任务等级同步 · 待实测', 'Normal-Quest Level Sync · Validation Pending') }}</b><small>{{ tx('尚未取得可区分普通任务、主线、命运篇章和教程的任务类型白名单，本构建不会安装该 Hook。', 'A proven quest-type allowlist separating normal, story, Fate, and tutorial quests is not yet available, so this build never installs this hook.') }}</small></span></label>
        <label class="qol-option is-candidate"><input v-model="config.returnWrightstone" type="checkbox" disabled /><span><b>{{ tx('重镶返还原祝福石 · 待实测', 'Return Replaced Wrightstone · Validation Pending') }}</b><small>{{ tx('尚未完成交易提交、旧石消耗和背包增量的现场三重验证，本构建不会安装该 Hook。', 'Field validation has not yet proven transaction commit, old-stone consumption, and inventory increment together, so this build never installs this hook.') }}</small></span></label>
...
.qol-option.is-candidate{cursor:not-allowed;opacity:.72}.qol-option.is-candidate input{cursor:not-allowed}
</style>
