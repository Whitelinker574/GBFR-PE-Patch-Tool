...
const cameraPresets = Object.freeze([
  { id: 'detail', mark: '近', nameZh: '角色细节', nameEn: 'Character Detail', copyZh: '贴近角色观察服装、武器与动作细节', copyEn: 'Move close for costume, weapon, and animation details', maxDistance: 2.8, targetHeight: 1.65, zoomStep: 0.012 },
  { id: 'game', mark: '原', nameZh: '游戏默认', nameEn: 'Game Default', copyZh: '还原游戏原本的镜头手感', copyEn: 'Restore the original camera feel', maxDistance: 4.8, targetHeight: 1.8, zoomStep: 0.05 },
  { id: 'comfort', mark: '舒', nameZh: '舒适探索', nameEn: 'Comfort Exploration', copyZh: '略微拉远，并让滚轮调节更细腻', copyEn: 'A little farther out with finer wheel control', maxDistance: 6, targetHeight: 1.8, zoomStep: 0.02 },
  { id: 'combat', mark: '战', nameZh: '战斗广角', nameEn: 'Combat Wide', copyZh: '扩大周边视野，更容易观察攻击范围', copyEn: 'Widen the view to read attack ranges more easily', maxDistance: 9, targetHeight: 2, zoomStep: 0.025 },
  { id: 'vista', mark: '远', nameZh: '远景观察', nameEn: 'Distant View', copyZh: '最大化环境与大型敌人的可见范围', copyEn: 'Maximize visibility of the environment and large enemies', maxDistance: 12, targetHeight: 2.2, zoomStep: 0.04 },
])
...
function usePreset(preset) {
  ...
  targetHeight.value = preset.targetHeight
  zoomStep.value = preset.zoomStep
}
...
<template>
  <div class="camera-lab ui-page-stack">
    <section class="camera-intro">
      ...
      <div class="camera-boundary"><b>{{ tx('只在确认匹配 2.0.2–2.0.5 时开启', 'Enabled Only After a 2.0.2–2.0.5 Match') }}</b><span>{{ tx('无法确认游戏版本时不会写入；停用后恢复开启前的数值', 'Nothing is written if the game version cannot be verified; disabling restores the values from before activation') }}</span></div>
    </section>
    ...
    <section class="camera-controls">
      <header class="controls-heading"><div><h3>{{ tx('第二步 · 选预设或手动调整', 'Step 2 · Choose a Preset or Adjust Manually') }}</h3><p>{{ tx('先选一种视角，再用下方滑轨微调；菲迪埃尔、齐格飞和夏洛特的角色镜头可能有独立覆盖，请以游戏内观察为准。', 'Choose a view, then fine-tune it below. Fediel, Siegfried, and Charlotta may apply character-specific camera overrides, so verify them in game.') }}</p></div></header>
      <div class="camera-preset-grid" role="list" :aria-label="tx('镜头体验预设', 'Camera experience presets')">
        <button v-for="preset in cameraPresets" :key="preset.id" class="camera-preset" type="button" @click="usePreset(preset)">
          <span>{{ preset.mark }}</span><b>{{ tx(preset.nameZh, preset.nameEn) }}</b><small>{{ tx(preset.copyZh, preset.copyEn) }}</small><em>{{ preset.maxDistance }} / {{ preset.targetHeight }} / {{ preset.zoomStep }}</em>
        </button>
      </div>
...
.camera-intro,.camera-setup,.camera-controls,.controls-heading,.camera-preset-grid,.parameter-grid,.camera-dock { width:100%; max-width:100%; min-width:0; }
...
.controls-heading { display:flex; align-items:flex-start; justify-content:space-between; gap:var(--space-4); margin-bottom:var(--space-4); }
.camera-preset-grid { display:grid; grid-template-columns:repeat(5,minmax(0,1fr)); gap:var(--space-2); margin-bottom:var(--space-4); }
.camera-preset { display:grid; grid-template-columns:32px minmax(0,1fr); grid-template-rows:auto auto auto; gap:2px var(--space-2); min-width:0; padding:var(--space-3); border:1px solid var(--border-default); border-radius:var(--radius-md); background:color-mix(in srgb,var(--surface-card) 88%,var(--accent-soft)); color:var(--text-primary); text-align:left; cursor:pointer; transition:border-color .16s ease,background .16s ease,transform .16s ease; }
.camera-preset:hover,.camera-preset:focus-visible { border-color:var(--accent-border); background:var(--surface-card-pop); transform:translateY(-1px); outline:none; }
.camera-preset > span { grid-row:1 / 4; display:grid; place-items:center; width:32px; height:32px; border:1px solid var(--accent-border); border-radius:50%; background:var(--accent-soft); color:var(--accent); font-family:var(--font-display); font-weight:var(--fw-bold); }
.camera-preset b { overflow:hidden; font-size:var(--fs-sm); text-overflow:ellipsis; white-space:nowrap; }.camera-preset small { min-height:2.7em; color:var(--text-secondary); font-size:var(--fs-xs); line-height:1.35; }.camera-preset em { color:var(--text-muted); font-family:var(--font-data); font-size:10px; font-style:normal; }
...
@container camera-lab (max-width:900px) { .camera-preset-grid { grid-template-columns:repeat(2,minmax(0,1fr)); } }
@container camera-lab (max-width:680px) { .camera-intro,.camera-setup { grid-template-columns:minmax(0,1fr); }.controls-heading,.camera-dock { align-items:stretch; flex-direction:column; }.dock-actions,.dock-actions .ui-btn { width:100%; } }
@container camera-lab (max-width:480px) { .camera-preset-grid { grid-template-columns:minmax(0,1fr); } }
