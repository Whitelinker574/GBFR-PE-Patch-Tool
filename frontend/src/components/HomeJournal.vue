...
const chapters = [
  { id: 'progression', icon: '⚔', title: '物品与武器', copy: '素材、武器等级与养成' },
  { id: 'sigil', icon: '◇', title: '因子、祝福与召唤石', copy: '生成、即时修改与合法性校验' },
  { id: 'runtime', icon: '✦', title: '游戏内实时修改', copy: '金币、MSP、药水、素材与任务掉落' },
  { id: 'chara', icon: '✎', title: '次数统计', copy: '角色使用次数与任务完成次数' },
]
</script>
    ...
    <section class="illustrated-journal">
      ...
      <div class="page-menu">
        ...
        <div class="small-tabs">
          <button @pointerenter="emit('warm', 'summon')" @focus="emit('warm', 'summon')" @click="emit('open', 'summon')">召唤石修改</button>
          <button @pointerenter="emit('warm', 'compatibility')" @focus="emit('warm', 'compatibility')" @click="emit('open', 'compatibility')">版本适配</button>
...
.project-heading { margin:0 13px 18px;padding-bottom:18px;border-bottom:1px solid rgba(155,112,51,.24) }.project-heading span { display:block;color:#755326;font-size:9px;font-weight:900;letter-spacing:.2em }.project-heading h1 { margin:7px 0 0;color:#514437;font-size:27px;font-weight:900;letter-spacing:.045em;text-shadow:0 1px #fff8e4 }.project-heading p { margin:6px 0 0;color:#65533d;font-size:11px;font-weight:800 }
.chapter-list { display:flex;flex-direction:column;gap:0;border-top:1px solid rgba(123,88,40,.18) }.chapter-ribbon { position:relative;width:100%;min-height:56px;display:grid;grid-template-columns:31px 1fr 17px;align-items:center;gap:10px;padding:7px 12px;border:0;border-bottom:1px solid rgba(123,88,40,.2);border-radius:0;color:#574b3d;background:rgba(249,240,214,.28);box-shadow:none;clip-path:none;text-align:left;cursor:pointer;transition:background-color .1s ease,color .1s ease }.chapter-ribbon:hover,.chapter-ribbon:focus-visible { color:#4d3f2d;background:rgba(255,249,229,.55);box-shadow:inset 3px 0 #9a7440 }.chapter-icon { width:27px;height:27px;display:grid;place-items:center;border:0;border-radius:0;color:#745a35;background:transparent;font-size:12px;font-weight:900;text-shadow:none }.chapter-ribbon strong,.chapter-ribbon small { display:block }.chapter-ribbon strong { font-size:12px;font-weight:900;letter-spacing:.01em }.chapter-ribbon small { margin-top:3px;color:#554838;font-size:9px;font-weight:800 }.chapter-ribbon b { color:#796444;font-size:16px;font-weight:900 }
.small-tabs { display:flex;align-items:center;gap:14px;margin:15px 13px 0;border-bottom:1px solid rgba(157,113,51,.24) }.small-tabs button { min-height:29px;padding:0 2px;border:0;border-bottom:2px solid transparent;border-radius:0;color:#6a4e25;background:transparent;font-size:9px;font-weight:900;cursor:pointer }.small-tabs button:hover { color:#765528;border-bottom-color:#9a7440;background:transparent }.small-tabs span { margin-left:auto;color:#6b5944;font-size:8px;font-weight:900 }
