...
const componentURL = new URL('./components/CT084RuntimeMonitor.vue', import.meta.url)
...
test('runtime monitor is a dedicated two-tab memory-monitoring page', () => {
  assert.ok(source, 'CT084RuntimeMonitor.vue must exist')
  assert.match(source, /data-page="ct084-runtime-monitor"/)
...
test('selected-item reading binds ExpectedSelectedAddr and becomes reselect-required after one read', () => {
  assert.ok(source, 'CT084RuntimeMonitor.vue must exist')
  assert.match(source, /CT084SelectedItemsEnableOwned/)
  assert.match(source, /CT084SelectedItemsStatusOwned/)
  assert.match(source, /CT084SelectedItemReadOwned\(ownerToken,\s*\{\s*kind,\s*expectedSelectedAddr\s*\}\s*\)/)
  assert.match(source, /consumeCT084SelectedCapture\(/)
...
test('the item panel is conspicuously read-only and exposes no inventory writer controls', () => {
  ...
  assert.doesNotMatch(source, /CT084SetEnabledOwned|CurrencySet|PotionSet|MonsterEnhance|QuestScore|ActionSpeed/)
...
test('the embedded page does not repeat the shell heading and keeps the narrow status badge intact', () => {
  ...
  assert.match(source, /data-page="ct084-runtime-monitor"[^>]*>\s*<section class="monitor-connection/)
