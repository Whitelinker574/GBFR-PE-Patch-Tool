...
test('dedicated editing hides the global portrait background while ordinary viewing retains it', () => {
	assert.match(viewer, /defineEmits\(\['status', 'editing-change', 'import-consumed'\]\)/)
...
test('single-loadout import constructs independent factors and blocks partial writes', () => {
  ...
  assert.match(importDialog, /武器替换技能[\s\S]*五个位置敏感技能槽[\s\S]*不改变武器等级/)
  assert.match(importDialog, /usesEndgameProgression[\s\S]*endgame-max/)
  assert.match(importDialog, /weaponEnhancement:\s*has\.value\.weaponEnhancement\s*&&\s*\(constructsWeapon\.value\s*\|\|\s*usesEndgameProgression\.value\)/)
  ...
  assert.match(source, /importedWeaponSkillSnapshot\.value = choices\.weaponSkills/)
  assert.match(importDialog, /MLv\{\{ masteryProgress \}\}/)
  assert.match(importDialog, /max="55"/)
  assert.match(source, /function selectImportTarget\(unitId\)/)
  assert.match(source, /hydrateFromTarget\(\{ preserveImport: true \}\)/)
  assert.match(source, /<LoadoutImportDialog :draft="importDraft" :slots="slots" :target-slot="targetSlot"/)
  assert.match(importDialog, /emit\('select-slot'/)
...
test('missing loadout scopes stay visible but cannot be selected or submitted', () => {
  ...
  assert.match(importDialog, /unavailable:\s*!has\.characterLevel/)
  assert.match(importDialog, /unavailable:\s*!has\.masterProgress/)
  assert.match(importDialog, /unavailable:\s*!has\.characterWeaponWrightstones/)
  assert.match(importDialog, /characterGrowth:\s*\(source\.value\.character\?\.enhancementPanel \|\| \[\]\)\.length === 2[\s\S]*enhancementNodes/)
  assert.match(importDialog, /:disabled="!has\.summons \|\| summonBlocked"/)
  assert.match(importDialog, /function toggle\(key\) \{\s*if \(!has\.value\[key\]\) return/)
  assert.match(importDialog, /for \(const key of availableKeys\.value\) next\[key\] = selected/)
  assert.match(importDialog, /\.import-choice\.unavailable\s*\{[^}]*cursor:not-allowed/is)
})
