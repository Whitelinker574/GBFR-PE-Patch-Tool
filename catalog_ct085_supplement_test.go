...
func TestCT085CelestialSigilsReachSaveAndLoadoutConstructors(t *testing.T) {
	...
	for hash, name := range want {
		item, ok := byHash[hash]
		if !ok {
			t.Errorf("CT 0.8.5 DLC factor %s (%s) is missing from the unified constructor catalog", name, hash)
			continue
		}
	...
	for _, entry := range sigilMemorySigils {
		...
		if _, ok := byHash[hash]; !ok {
			t.Errorf("0.8.5 supplemental factor %s (%s) is missing", entry.Name, hash)
		}
		if ctHashToName[entry.Hash] == "" && ctHashToNameEN[entry.Hash] == "" {
			t.Errorf("0.8.5 supplemental factor %s (%s) is absent from the CT name tables", entry.Name, hash)
		}
	}
}

func TestCT085SupplementalBlessingTraitsReachSaveAndMemoryEditors(t *testing.T) {
	...
	for hash, name := range want {
		...
		if trait == nil {
			t.Errorf("CT 0.8.5 blessing trait %s (%s) is missing", name, hash)
			continue
		}
	...
	for _, entry := range sigilMemoryTraits {
		...
		if trait == nil {
			t.Errorf("0.8.5 supplemental blessing trait %s (0x%08X) is missing", entry.Name, entry.Hash)
		}
		if ctHashToName[entry.Hash] == "" && ctHashToNameEN[entry.Hash] == "" {
			t.Errorf("0.8.5 supplemental blessing trait %s (0x%08X) is absent from the CT name tables", entry.Name, entry.Hash)
		}
	}
}

func TestUnifiedFactorCatalogHasNoDuplicateHashesAfterCT085Supplement(t *testing.T) {
	items, err := NewSigilGen().GetSigilList()
	if err != nil {
		t.Fatal(err)
	}
...
func TestCT085SupplementalFactorNamesStayLanguageIsolated(t *testing.T) {
	previousLanguage := getCurrentLanguage()
	defer setCurrentLanguage(previousLanguage)
