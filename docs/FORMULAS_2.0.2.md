...
2026-07-21 已在安装完整 DLC 2.0.2 且实际运行游戏的 Windows 机器上完成首轮现场校准。静态 EXE、解包表、可恢复的配装预设实验、游戏画面与只读运行时回读分别记录；只有相互吻合的项目才提升证据等级。现场机器路径、进程 ID、模块基址和绝对地址不进入提交或证据包。
...
现场进一步确认：存储中的 `10/10/10/20` 是四组数组的结构容量，不能反推当前解锁等级。伊欧 Master Lv50 的 50 节点预设与移除一个 EX 节点后的 49 节点预设，游戏分别显示 `10/10/10/20` 与 `10/10/10/19`。但“专精系统尚未开放”和“已开放且总 MSP 为 0”仍缺少独立进度标志或成对 UI 样本，现阶段必须保留为未闭环边界。
...
3. 枚举链表节点：`node+0x08` 为 next、`+0x10` 为 map key、`+0x30` 为状态指针。
4. 目录角色 hash 与现场 map key/runtime ID 精确相同；伊欧为 `4D0A60C3`。`status+0x59F0` 在伊欧对象中为 `00000000`，因此它只是候选诊断字段，不能否决精确 map-key 匹配。
5. `status+0x5EBC` 的 ready 与 `+0x5EBE` 的 eligibility 进入诊断输出，但不再作为枚举阶段的硬过滤条件。最终接受仍要求有效状态指针、精确目录映射、字段边界和连续稳定读取。
...
| 角色 | 目录 hash / runtime ID | HP | 攻击 | 暴击 | 昏厥 | 证据 |
|---|---|---:|---:|---:|---:|---|
| 伊欧 | `4D0A60C3` | `132977` | `111259` | `124` | `306` | 游戏画面一致；每项连续 3 次位级稳定 |
| 姬塔 | `A4ACBA76` | `102131` | `25887` | `115` | `216` | 游戏画面一致；每项连续 3 次位级稳定 |

伊欧另一把武器的独立现场样本为 `134588 / 71468 / 123 / 329`，切回原配装后恢复。角色对象中 `+0x10` 的伊欧原始值约为 `30.599998`，显示 `306`；这证明该字段是 raw `float32`，显示端乘 `10`，而不是把屏幕整数直接存回对象。

旧版工具在配装切换后曾继续显示上一次会话的 `132977 / 111259 / 124 / 306`，同时一个全新的只读进程句柄已经能从同一 map 状态对象读到当前 `159573 / 111259 / 108 / 306`。根因是前端用单一 `observing` 布尔值跨采样会话复用：旧请求可以抑制或覆盖新会话。现改为按 session token 与递增 epoch 隔离，并在 attach/close/unmount 时使旧请求失效。

### 专精节点现场单变量实验

所有实验都通过游戏自身的“读取该配装”应用预设；预设除目标节点外逐字段相同。每个已闭环项目执行 A1/B1/A2/B2，观察游戏画面和只读最终状态对象。实验结束后整组存档按原始备份恢复，并校验 SHA-256。

| 节点 | 原表文本/动作值 | A | B | 可重复最终差值 | 结论 |
|---|---|---|---|---:|---|
| `1F52146F` | 昏厥值 `+0.4`，raw f32 `0.4` | `159573 / 111259 / 108 / 306` | `159573 / 111259 / 108 / 302` | 昏厥 `+4` | 原始专精值进入 raw 状态尺度，最终人物面板显示 `×10` |
| `A66398F9` | 最大 HP `+15000` | `159573 / 111259 / 108 / 306` | `140673 / 111259 / 108 / 306` | HP `+18900` | 当前配装中平坦 `15000` 进入 `×1.26` HP 层；不能普遍硬编码 `18900` |
| `E0E6FF0C` | 专注效果范围内获得减伤 | 四项同 A | 四项同 A | 城镇四项 `0` | 仅为负观察；仍需训练场激活专注范围后的受击 A/B 才能闭环减伤字段/结算公式 |

`E0E6FF0C` 的 hash 也没有直接出现在同一状态对象前 `0x6000` 字节的受限扫描窗口中。这不证明效果不存在；它可能存在于关联子对象、Buff 容器或受击结算临时参数。证据文件明确记录需要补采的“受击前 HP / 受击后 HP / 伤害 / 当前 Buff”。
