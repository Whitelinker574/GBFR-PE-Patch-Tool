...
| SkillId | 主加护 | Lv1 → Lv15 的权威数值 |
|---|---|---|
| `SKILL_320_00` | 天星之炼 | 攻击 `16→30%`，上限 `50→120%`，HP 条件 `≤25%` |
| `SKILL_321_00` | 天星之煌 | 攻击 `6→20%`，上限 `2→70%`，HP 条件 `≥75%` |
| `SKILL_322_00` | 天星之界 | 最大 HP `-30%`，上限 `1→70%`；中间列为 `1,5,10,…,65,70` |
| `SKILL_323_00` | 天星之焰 | OVERDRIVE 敌人伤害上限 `10→50%` |
| `SKILL_324_00` | 天星之雪 | 破防/连锁条件伤害 `1→15%` |
| `SKILL_325_00` | 浪迹天涯 | 指定级别任务内攻击/上限 `10→50%`，防御 `1→5%` |
| `SKILL_326_00` | 天星之止息 | 造成伤害 `1→10%`，同时禁用同伴 HP 回复 |
| `SKILL_327_00` | 分歧 | 持续回复 `0.6→2%`，净化概率 `1→15%` |

`320/321/323/324/325/327` 依赖当前 HP、敌人状态、任务类型或概率触发，只展示官方明细，不进入静态总计；`322/326` 为无条件效果。直接技能 hash 与旧工具可能写入的物品 hash 共 16 个读取别名复用同一曲线，但只有 82 个 SkillId 能进入正常写入目录。
