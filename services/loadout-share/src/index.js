...
const PROJECT_RELEASE_URL = 'https://github.com/Whitelinker574/GBFR-PE-Patch-Tool/releases/latest'
...
function projectReleaseLink(lang) {
  return `<a class="repo-link" href="${PROJECT_RELEASE_URL}" target="_blank" rel="noopener noreferrer"><span aria-hidden="true">↗</span>${lang === 'en' ? 'Download App on GitHub' : 'GitHub 下载应用'}</a>`
}
...
function catalogCardPreview(preview) {
  if (!preview) return {}
  const skills = items => (items || []).slice(0, 3).map(item => ({ name: item?.name || '', level: Number(item?.level) || 0 }))
  return {
    masteryLabel: preview.masteryLabel || '',
    masteryCount: Number(preview.masteryCount) || 0,
    weaponSkills: skills(preview.weaponSkills).slice(0, 2),
    sigils: skills(preview.sigils),
  }
}
...
async function communityAction(request, env, code, action) {
  ...
  try {
    if (action === 'like') {
      const liked = typeof body.liked === 'boolean' ? body.liked : true
      if (liked) await env.COMMUNITY_DB.prepare('INSERT OR IGNORE INTO likes (code, visitor_key) VALUES (?, ?)').bind(code, visitor).run()
      else await env.COMMUNITY_DB.prepare('DELETE FROM likes WHERE code = ? AND visitor_key = ?').bind(code, visitor).run()
      ...
      const entry = await env.COMMUNITY_DB.prepare('SELECT likes_count FROM loadouts WHERE code = ?').bind(code).first()
      return jsonResponse({ enabled: true, liked, likes: Number(entry?.likes_count || 0) }, 200, { 'Cache-Control': 'no-store' })
    }
...
function rosterNav(activeSlug = '', lang = 'zh') {
  return CHARACTER_ROSTER.map(character => {
    ...
    return `<a class="roster-chip${character.slug === activeSlug ? ' active' : ''}" href="${withLanguage(`/c/${character.slug}`, lang)}" title="${name}"${character.slug === activeSlug ? ' aria-current="page"' : ''}><img src="/assets/avatars/${character.iconFile}" alt="${name}" width="46" height="46" loading="lazy" decoding="async"><span>${name}</span></a>`
  }).join('')
}
...
function showcaseStyles() {
  ...
  *{scrollbar-width:thin;scrollbar-color:rgba(126,89,40,.38) transparent}
  ...
  .masthead-tools{display:flex;flex-wrap:wrap;align-items:flex-end;justify-content:flex-end;gap:10px 14px}.masthead-tools p{flex:1 1 280px}.repo-link{min-height:32px;display:inline-flex;flex:none;align-items:center;gap:6px;padding:0 10px;border:1px solid rgba(105,76,37,.28);border-radius:5px;background:rgba(255,253,247,.74);color:#6e4e28;font-size:10px;font-weight:800;white-space:nowrap;transition:background-color .16s,border-color .16s,transform .16s}.repo-link span{font-size:13px}.repo-link:hover{border-color:#896331;background:#fffaf0;transform:translateY(-1px)}.repo-link:focus-visible{outline:2px solid #98703a;outline-offset:2px}.language-switch{display:inline-flex;flex:none;padding:2px;border:1px solid rgba(105,76,37,.25);border-radius:5px;background:rgba(255,253,247,.72)}.language-switch a{min-width:42px;padding:5px 8px;border-radius:3px;color:#7f5d31;font-size:10px;font-weight:800;text-align:center}.language-switch a.active{background:#896331;color:#fffaf0}
...
function catalogScript(_origin, fixedCharacter = '', lang = 'zh') {
  return `<script>
  ...
  const t=${JSON.stringify(lang === 'en' ? {
    ...
    mastery:'Mastery', nodes:'nodes', publicCount:'public loadouts', empty:'No public loadouts yet', like:'Like', sortTime:'Newest', sortName:'Name', sortLikes:'Likes'
  } : {
    unnamed:'未命名配装', weapon:'装备武器', unknownWeapon:'未标注', emptySummary:'等待完整配装摘要',
    mastery:'专精', nodes:'节点', publicCount:'套公开配装', empty:'暂时还没有公开配装', like:'点赞', sortTime:'最新', sortName:'名称', sortLikes:'点赞'
  })};
  const esc=s=>String(s??'').replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]));
  const label=s=>String(s??'').replaceAll('Catastrophe','浩劫').replaceAll('DMG Cap','伤害上限').replace(/(?:\||｜)HP$/i,'｜体力');
  const likedStorageKey='gbfr-share-liked-codes';
  function readLikedCodes(){try{return new Set(JSON.parse(localStorage.getItem(likedStorageKey)||'[]'))}catch{return new Set()}}
  const likedCodes=readLikedCodes();
  function rememberLike(code,liked){if(liked)likedCodes.add(code);else likedCodes.delete(code);localStorage.setItem(likedStorageKey,JSON.stringify([...likedCodes]))}
  function paintLike(button,liked,likes){const count=Math.max(0,Number(likes)||0);button.querySelector('b').textContent=count;button.querySelector('span').textContent=liked?'♥':'♡';button.classList.toggle('liked',liked);button.setAttribute('aria-pressed',String(liked));button.setAttribute('aria-label',t.like+' '+count)}
  function renderCard(x){
    ...
    const likes=Math.max(0,Number(x.likes)||0);
    const liked=likedCodes.has(x.code);
    return '<article class="loadout-card" style="--card-accent:'+esc(x.accent||'#896331')+'"><a class="loadout-card-main" data-code="'+esc(x.code)+'" href="/s/'+esc(x.code)+'?lang='+lang+'"><header class="loadout-card-head"><img src="'+esc(x.avatarPath)+'" alt="'+esc(x.characterName)+'" loading="lazy" decoding="async"><div><small>'+esc(x.characterName)+'</small><h3>'+esc(label(x.title||t.unnamed))+'</h3></div></header><div class="loadout-weapon"><small>'+t.weapon+'</small><b>'+esc(x.weaponName||t.unknownWeapon)+'</b></div><div class="card-tags">'+(tags.join('')||'<span>'+t.emptySummary+'</span>')+'</div></a><footer class="card-footer"><span>'+code+'</span><span>'+new Date(x.createdAt).toLocaleDateString(lang==='en'?'en-US':'zh-CN')+'</span><button class="card-like'+(liked?' liked':'')+'" type="button" data-code="'+esc(x.code)+'" aria-label="'+t.like+' '+likes+'" aria-pressed="'+liked+'"><span aria-hidden="true">'+(liked?'♥':'♡')+'</span><b>'+likes+'</b></button></footer></article>';
  }
...
function communityScript(_origin, code, lang = 'zh') {
  ...
  return `<script>(async()=>{const key='gbfr-share-visitor',likedKey='gbfr-share-liked-codes',code='${code}';let visitor=localStorage.getItem(key);if(!visitor){visitor=crypto.randomUUID();localStorage.setItem(key,visitor)}const esc=s=>String(s).replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]));function readLiked(){try{return new Set(JSON.parse(localStorage.getItem(likedKey)||'[]'))}catch{return new Set()}}const likedCodes=readLiked(),like=document.querySelector('#like');function remember(liked){if(liked)likedCodes.add(code);else likedCodes.delete(code);localStorage.setItem(likedKey,JSON.stringify([...likedCodes]))}function paint(liked,likes){like.querySelector('.community-heart').textContent=liked?'♥':'♡';like.querySelector('.community-like-count').textContent=Math.max(0,Number(likes)||0);like.classList.toggle('liked',liked);like.setAttribute('aria-pressed',String(liked))}async function refresh(){try{const r=await fetch('/api/v1/loadouts/${code}/community');const d=await r.json();paint(likedCodes.has(code),d.likes||0);document.querySelector('#comments').innerHTML=(d.comments||[]).map(c=>'<article class="comment"><small>'+esc(c.author||'${author}')+'</small><p>'+esc(c.body)+'</p></article>').join('');if(!d.enabled)document.querySelector('#community-note').textContent='${disabled}'}catch{}}like.onclick=async()=>{if(like.dataset.pending==='true')return;const previousLiked=like.getAttribute('aria-pressed')==='true',previousLikes=Math.max(0,Number(like.querySelector('.community-like-count').textContent)||0),liked=!previousLiked;paint(liked,previousLikes+(liked?1:-1));like.dataset.pending='true';like.classList.add('pending');like.setAttribute('aria-busy','true');try{const r=await fetch('/api/v1/loadouts/${code}/like',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({visitorKey:visitor,liked})}),d=await r.json();if(!r.ok)throw new Error(d.error||'Like failed');remember(Boolean(d.liked));paint(Boolean(d.liked),d.likes);like.removeAttribute('title')}catch(error){paint(previousLiked,previousLikes);like.title=String(error.message||error)}finally{delete like.dataset.pending;like.classList.remove('pending');like.removeAttribute('aria-busy')}};document.querySelector('#send').onclick=async()=>{const box=document.querySelector('#comment');if(!box.value.trim())return;await fetch('/api/v1/loadouts/${code}/comments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({visitorKey:visitor,body:box.value,author:'${author}'})});box.value='';refresh()};refresh()})()</script>`
}

function detailScript(_origin, code, lang = 'zh') {
  return `<script>
  (async()=>{
    const lang=${JSON.stringify(lang)};
    const en=lang==='en';
    ...
    try{
      const r=await fetch('/api/v1/loadouts/${code}/meta?lang='+lang);
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && metaMatch) {
      ...
      return jsonResponse(metadata, 200, { 'Cache-Control': 'public, max-age=60, stale-while-revalidate=300' })
    }
