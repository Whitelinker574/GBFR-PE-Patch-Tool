...
function catalogScript(_origin, fixedCharacter = '', lang = 'zh') {
  ...
  const t=${JSON.stringify(lang === 'en' ? {
    ...
    mastery:'Mastery', nodes:'nodes', publicCount:'public loadouts', empty:'No public loadouts yet', like:'Like'
  } : {
    unnamed:'未命名配装', weapon:'装备武器', unknownWeapon:'未标注', emptySummary:'等待完整配装摘要',
    mastery:'专精', nodes:'节点', publicCount:'套公开配装', empty:'暂时还没有公开配装', like:'点赞'
  })};
  ...
  async function likeCard(button){if(button.disabled)return;button.disabled=true;try{const response=await fetch('/api/v1/loadouts/'+encodeURIComponent(button.dataset.code)+'/like',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({visitorKey:visitorKey()})});const data=await response.json();if(!response.ok)throw new Error(data.error||'Like failed');button.querySelector('b').textContent=Math.max(0,Number(data.likes)||0);button.querySelector('span').textContent='♥';button.classList.add('liked');button.setAttribute('aria-pressed','true');button.setAttribute('aria-label',t.like+' '+(data.likes||0))}catch(error){button.title=String(error.message||error)}finally{button.disabled=false}}
  async function load(){
