...
type ct084FakeWrite struct {
	addr uintptr
	data []byte
}

type ct084FakeMemory struct {
	data           map[uintptr][]byte
	writes         []ct084FakeWrite
	reads          int
	writeCalls     int
...
func (m *ct084FakeMemory) ReadCode(addr uintptr, size int) ([]byte, error) {
	m.reads++
...
func (m *ct084FakeMemory) WriteCode(addr uintptr, data []byte) error {
	m.writeCalls++
	m.writes = append(m.writes, ct084FakeWrite{addr: addr, data: append([]byte(nil), data...)})
...
func ct084TestSites() []ct084PatchSiteLease {
...
func TestCT084PatchSingleSiteEnableDisable(t *testing.T) {
	sites := ct084TestSites()[:1]
	memory := newCT084FakeMemory(map[uintptr][]byte{sites[0].Address: sites[0].Original})

	if err := installCT084PatchSites(memory, sites); err != nil {
		t.Fatalf("installCT084PatchSites() error = %v", err)
	}
	...
	if err := restoreCT084PatchSites(memory, sites); err != nil {
		t.Fatalf("restoreCT084PatchSites() error = %v", err)
	}
...
func TestCT084PatchThreeSiteEnable(t *testing.T) {
	sites := ct084TestSites()
...
func TestCT084PatchPreflightRejectsLaterForeignBytesBeforeFirstWrite(t *testing.T) {
	...
	if err := installCT084PatchSites(memory, sites); err == nil {
		t.Fatal("installCT084PatchSites() error = nil, want preflight rejection")
	}
...
func TestCT084PatchPreflightRejectsActualAddressOverlap(t *testing.T) {
	sites := []ct084PatchSiteLease{
		{Address: 0x1000, Original: []byte{1, 2}, Patch: []byte{3, 4}},
		{Address: 0x1001, Original: []byte{2, 5}, Patch: []byte{4, 6}},
	}
	memory := newCT084FakeMemory(map[uintptr][]byte{0x1000: {1, 2}, 0x1001: {2, 5}})

	if err := installCT084PatchSites(memory, sites); err == nil {
		t.Fatal("installCT084PatchSites() error = nil, want overlap rejection")
	}
	if len(memory.writes) != 0 {
		t.Fatalf("writes = %v, want none", memory.writes)
	}
...
func TestCT084PatchSecondWriteFailureRollsBackInReverse(t *testing.T) {
	...
	err := installCT084PatchSites(memory, sites)
	if err == nil {
		t.Fatal("installCT084PatchSites() error = nil")
	}
...
func TestCT084PatchWriteReadbackMismatchRollsBackAllSites(t *testing.T) {
	sites := ct084TestSites()[:2]
	memory := newCT084FakeMemory(map[uintptr][]byte{
		sites[0].Address: sites[0].Original,
		sites[1].Address: sites[1].Original,
...
func TestCT084PatchFinalReadbackFailureRollsBackAllSites(t *testing.T) {
	sites := ct084TestSites()[:2]
	memory := newCT084FakeMemory(map[uintptr][]byte{
		sites[0].Address: sites[0].Original,
		sites[1].Address: sites[1].Original,
...
func TestCT084PatchRestoreForeignBytesNeverOverwrites(t *testing.T) {
	site := ct084TestSites()[0]
	memory := newCT084FakeMemory(map[uintptr][]byte{site.Address: {0xCC}})

	err := restoreCT084PatchSites(memory, []ct084PatchSiteLease{site})
	if !errors.Is(err, errLiveMemoryRollbackUnproven) {
...
func TestCT084PatchRestoreContinuesAfterOneSiteFailure(t *testing.T) {
	sites := ct084TestSites()[:2]
	memory := newCT084FakeMemory(map[uintptr][]byte{
	...
	err := restoreCT084PatchSites(memory, sites)
	if !errors.Is(err, errLiveMemoryRollbackUnproven) {
		t.Fatalf("restore error = %v, want errLiveMemoryRollbackUnproven", err)
	}
	if !bytes.Equal(memory.data[sites[0].Address], sites[0].Original) {
...
func TestCT084PatchSiteCopiesAreDefensive(t *testing.T) {
	site := ct084PatchSiteLease{Address: 0x1000, Original: []byte{1}, Patch: []byte{2}}
	copySite := cloneCT084PatchSiteLease(site)
...
func TestCT084PatchOwnedLeaseRejectsStaleEmptyAndReplacedProcessBeforeIO(t *testing.T) {
	lease := ct084PatchLease{
		FeatureID:  "ct084-1",
		OwnerToken: "current",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if err := validateCT084OwnedLease(lease, test.token, test.process); err == nil {
				t.Fatal("validateCT084OwnedLease() error = nil")
			}
		})
	}
}

func TestCT084PatchConflictAndActiveAddressOverlap(t *testing.T) {
	feature := CT084Feature{ID: "ct084-new", Conflicts: []string{"ct084-old"}}
	leases := map[string]ct084PatchLease{
		"ct084-old": {
			FeatureID: "ct084-old",
			Sites:     []ct084PatchSiteLease{{Address: 0x2000, Original: []byte{1, 2}, Patch: []byte{3, 4}}},
		},
	}
	if conflict := findCT084CatalogConflict(feature, leases); conflict != "ct084-old" {
		t.Fatalf("conflict = %q, want ct084-old", conflict)
	}
	if overlap := findCT084ActiveAddressOverlap([]ct084PatchSiteLease{{Address: 0x2001, Original: []byte{2}, Patch: []byte{4}}}, leases, ""); overlap == "" {
		t.Fatal("actual address overlap was not detected")
	}
}

func TestCT084PatchRestoreOwnedLeasesUsesReverseFeatureAndSiteOrder(t *testing.T) {
	firstSites := []ct084PatchSiteLease{
		{Address: 0x1000, Original: []byte{1}, Patch: []byte{11}},
		{Address: 0x1100, Original: []byte{2}, Patch: []byte{12}},
	}
	secondSites := []ct084PatchSiteLease{
	...
	process := processInstanceID{PID: 42, Created: 100}
	leases := map[string]ct084PatchLease{
		"first":  {FeatureID: "first", OwnerToken: "owner", Process: process, State: ct084PatchEnabled, Sites: firstSites},
		"second": {FeatureID: "second", OwnerToken: "owner", Process: process, State: ct084PatchEnabled, Sites: secondSites},
	}
	order := []string{"first", "second"}

	if err := restoreAllCT084PatchLeases(memory, leases, &order, process, "owner"); err != nil {
		t.Fatalf("restoreAllCT084PatchLeases() error = %v", err)
	}
...
func TestCT084PatchRestoreOwnedLeasesRetainsUnprovenLeaseAndOrder(t *testing.T) {
	site := ct084PatchSiteLease{Address: 0x1000, Original: []byte{1}, Patch: []byte{2}}
	memory := newCT084FakeMemory(map[uintptr][]byte{site.Address: {0xCC}})
	process := processInstanceID{PID: 42, Created: 100}
	leases := map[string]ct084PatchLease{
		"feature": {FeatureID: "feature", OwnerToken: "owner", Process: process, State: ct084PatchRecovery, Sites: []ct084PatchSiteLease{site}},
	}
	order := []string{"feature"}

	err := restoreAllCT084PatchLeases(memory, leases, &order, process, "owner")
	if !errors.Is(err, errLiveMemoryRollbackUnproven) {
...
func TestCT084PatchRestoreOwnedLeasesRejectsMissingOrderEntryWithoutIO(t *testing.T) {
	process := processInstanceID{PID: 42, Created: 100}
	lease := ct084PatchLease{
		FeatureID: "feature", OwnerToken: "owner", Process: process, State: ct084PatchRecovery,
		Sites: []ct084PatchSiteLease{{Address: 0x1000, Original: []byte{1}, Patch: []byte{2}}},
	}
	memory := newCT084FakeMemory(map[uintptr][]byte{0x1000: {2}})
	leases := map[string]ct084PatchLease{"feature": lease}
	order := []string{}
...
func TestCT084PatchStatusesUseDefensiveNonNilArraysAndNeverReportForeignAsEnabled(t *testing.T) {
	feature := CT084Feature{ID: "feature", Sites: []CT084PatchSite{{EnableBytes: []byte{2}}}}
	process := processInstanceID{PID: 42, Created: 100}
	lease := ct084PatchLease{
		FeatureID: "feature", OwnerToken: "owner", Process: process, State: ct084PatchEnabled,
		Sites: []ct084PatchSiteLease{{Address: 0x140001000, RVA: 0x1000, Original: []byte{1}, Patch: []byte{2}}},
	}
	memory := newCT084FakeMemory(map[uintptr][]byte{0x140001000: {0xCC}})
	statuses := buildCT084FeatureStatuses(
		[]CT084Feature{feature}, memory, true, "owner", process,
		map[string]ct084PatchLease{"feature": lease},
	)
	if len(statuses) != 1 {
	...
	missing := buildCT084FeatureStatuses([]CT084Feature{{ID: "missing"}}, memory, true, "owner", process, nil)[0]
...
func TestCT084PatchStatusesRejectForeignLeaseBeforeMemoryIO(t *testing.T) {
	process := processInstanceID{PID: 42, Created: 100}
	lease := ct084PatchLease{
		FeatureID: "feature", OwnerToken: "other", Process: process, State: ct084PatchEnabled,
		Sites: []ct084PatchSiteLease{{Address: 0x1000, RVA: 0x1000, Original: []byte{1}, Patch: []byte{2}}},
	}
	memory := newCT084FakeMemory(map[uintptr][]byte{0x1000: {2}})

	status := buildCT084FeatureStatuses(
		[]CT084Feature{{ID: "feature"}}, memory, true, "presented", process,
		map[string]ct084PatchLease{"feature": lease},
	)[0]
...
func TestCharaCT084LeaseBlocksAttachAcquireAndCountsAsActive(t *testing.T) {
	app := &App{
		ct084PatchLeases: map[string]ct084PatchLease{"feature": {FeatureID: "feature", OwnerToken: "owner"}},
		ct084PatchOrder:  []string{"feature"},
	}
	if !app.hasActiveRuntimeHookLeaseLocked() {
		t.Fatal("CT084 recovery lease is not included in active runtime hooks")
	}
	if _, err := app.CharaAttach(); err == nil {
		t.Fatal("CharaAttach rotated connection while CT084 lease existed")
	}
	if _, err := app.CharaAcquire(1); err == nil {
		t.Fatal("CharaAcquire rotated owner while CT084 lease existed")
	}
}

func TestCharaReleaseDeadProcessDropsOnlyPresentedCT084Owner(t *testing.T) {
	process := processInstanceID{PID: 42, Created: 100}
	app := &App{
		...
		ct084PatchOrder: []string{"current-feature", "other-feature"},
	}

	if err := app.CharaRelease("current"); err != nil {
		t.Fatal(err)
	}
	if _, ok := app.ct084PatchLeases["current-feature"]; ok {
		t.Fatal("dead-process release retained presented owner's lease")
	}
	if _, ok := app.ct084PatchLeases["other-feature"]; !ok {
...
func TestCharaDetachLockedRestoresCT084WithoutTakingGlobalLockAgain(t *testing.T) {
	...
	if !blockCallsSelector(body, "a", "restoreAllCT084PatchesLocked") {
		t.Fatal("charaDetachLocked does not restore CT084 leases")
	}
