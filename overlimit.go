...
func (a *App) OverLimitSetSlot(update OverLimitUpdate) (OverLimitStatus, error) {
	...
	if err := snapshotBeforeLiveSaveChange(fmt.Sprintf("角色上限突破槽位%d写入前自动备份", update.Index+1)); err != nil {
...
// validateRemoteFunctionStart is a heuristic guard against version drift before
// we CreateRemoteThread into a hardcoded game-function RVA (e.g. the save
// function at 0x79D820). We do not ship the game binary, so an exact prologue
// baseline is unavailable; instead we reject targets that clearly are not a
// function entry — a null address, unreadable memory, or a leading padding byte
// (0xCC int3 / 0x00) / all-padding block, which is what a stale RVA lands in
// when the game updates. It cannot catch a stale RVA that lands mid-function;
// that would need a per-version prologue baseline.
func (a *App) validateRemoteFunctionStart(fn uintptr, label string) error {
	...
	if buf[0] == 0xCC || buf[0] == 0x00 {
		return fmt.Errorf("%s 序言字节异常(0x%02X)，疑似游戏版本不匹配，已中止调用: %s", label, buf[0], bytesToHex(buf))
	}
	...
	if allZero || allInt3 {
		return fmt.Errorf("%s 序言为空/填充，疑似游戏版本不匹配，已中止调用", label)
	}
	return nil
}
