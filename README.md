...
<p align="center">
  <a href="https://github.com/Whitelinker574/GBFR-PE-Patch-Tool/releases/latest"><strong>下载 v1.91.20</strong></a> ·
...
| 自动保存每场任务的队伍配装，或导入 GBFR Logs 记录 | `内存监测（只读）` → `运行监测` | **启动游戏进入任务，或选择 Logs 数据库** |
...
| 页面 | 可以做什么 | 操作要点 |
| --- | --- | --- |
| 配装预设（查看与写入） | 查看完整配装、编辑并分项导入，生成离线长码或在线短码 | 可多选导入因子、技能、专精/等级、武器与五技能、祝福、召唤石、角色强化和四槽上限突破；角色强化只增不减 |
...
| 页面 | 可以做什么 | 操作要点 |
| --- | --- | --- |
| 运行监测 | 开启常驻后台的角色配装检测，按场次保存队伍配装；批量读取 GBFR Logs 数据库；查看队伍、碧的小红龙和当前选中的素材/关键物品 | 后台检测开启一次后可切走页面；每名角色都可预览、导出、上传或部署到指定存档槽位；选中物品 Hook 会在安全断开或离页时恢复 |
...
1. 进入 `内存监测（只读）` → `运行监测`，开启“角色配装检测”。检测会常驻后台并自动重连，不需要每场任务回来刷新。
2. 正常进入任务。队伍连续稳定后，本场配装会按玩家和队员归档到本机；同场后续补齐的队员会更新原记录，不会重复创建场次。
3. 在任务记录中可逐个预览角色完整配装，随后导出文件、复制配装码、上传在线图鉴，或转到配装预设选择存档和槽位部署。
4. 已使用 GBFR Logs 的用户也可选择其 `logs.db` 批量读取历史角色配装。数据库只读打开，不会修改 Logs 记录。

运行时和 Logs 只能导入实际捕获到的范围；缺失的范围会在导入弹窗中置灰。捕获配装按毕业用途归一化角色、武器、因子、祝福和召唤石等级，但保留真实的因子组合、专精路线、武器技能和角色技能。
