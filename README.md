...
- **配装预设** — 读取并写入单个角色的单套配装，完整配置武器、12 个因子、4 个角色技能和三方向三阶段专精；可从真实背包选择 4 个不重复召唤石并可选同步到存档全局装备槽（非每套配装独立字段）；支持单套 JSON 导入/导出，并在确认写入前显示与后端实际写入规则一致的逐槽合规报告
...
- 召唤石实体图标与副参数已有覆盖，但主加护定义仍有限；未收录效果显示哈希并明确“不计入”，不会猜值。
