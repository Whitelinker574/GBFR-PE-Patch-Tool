...
<p align="center">
  <a href="https://share.whitelinker.top"><strong>◆ 打开 GBFR 在线配装图鉴 · 浏览、搜索、预览和分享社区配装 ◆</strong></a>
</p>
...
| 自动保存每场任务的队伍配装 | `内存监测（只读）` → `角色配装检测` | **启动游戏并进入任务** |
| 导入 GBFR Logs 数据库或 Relink Logs 角色 JSON | `存档修改（离线）` → `配装预设（查看与写入）` → `从 GBFR Logs 批量获取队伍配装` | **读取记录不需要游戏；写入存档前退出游戏** |
| 查看队伍状态或校准角色最终属性 | `内存监测（只读）` | **启动游戏并进入稳定场景** |
| 检查版本、备份或恢复游戏 EXE | `工具与设置` | 按页面提示 |

离线、实时、只读是三套不同工作流。离线页面修改存档文件；实时页面连接当前游戏进程并写入；只读页面不改变角色、物品或存档数据。公式采样器只查询和读取内存；角色配装检测为读取当前选中物品会临时安装捕获 Hook，并在安全断开或离开页面时恢复原字节。
...
![首页的四个工作区](docs/screenshots/home-v19116-annotated.png)
...
| 页面 | 可以做什么 | 操作要点 |
| --- | --- | --- |
| 配装预设（查看与写入） | 查看完整配装、编辑并分项导入，生成离线长码或在线短码；从 Logs 数据库或角色 JSON 批量整理外部队伍配装 | 可多选导入因子、技能、专精/等级、武器与五技能、祝福、召唤石、角色强化和四槽上限突破；角色强化只增不减 |
...
| 页面 | 可以做什么 | 操作要点 |
| --- | --- | --- |
| 角色配装检测 | 开启常驻后台检测，进入任务后按场次自动保存队伍配装；查看队伍、碧的小红龙和当前选中的素材/关键物品 | 后台检测开启一次后可切走页面；每名角色都可预览、导出、上传或部署到指定存档槽位；选中物品 Hook 会在安全断开或离页时恢复 |
...
![完整配装编辑器](docs/screenshots/loadout-editor-v19116.png)

**这张图看什么：** 左栏对照配装估算与游戏真实回读，中栏编辑 12 个独立因子并保留官方图标；继续横向查看右栏可展开合并后的因子、武器、祝福、召唤石与专精技能等级及来源。顶部“导入单套”会先打开分项范围确认，不会直接覆盖整份存档。

![在线短码与离线长码](docs/screenshots/loadout-share-v19116-annotated.png)

**分享配装：** 点“分享码”可生成 16 位短码和链接；默认同步到在线图鉴，也可关闭上传，仅生成无需联网的离线长码。接收后仍会进入分项导入确认。

### 自动收集队伍配装

1. 进入 `内存监测（只读）` → `角色配装检测`，点击“开启后台检测”。检测会常驻后台并自动重连，不需要每场任务回来刷新。
2. 正常进入任务。队伍连续稳定后，本场配装会按玩家和队员归档到本机；同场后续补齐的队员会更新原记录，不会重复创建场次。
3. 在任务记录中可逐个预览角色完整配装，随后导出文件、复制配装码、上传在线图鉴，或转到配装预设选择存档和槽位部署。

后台检测只保存在任务中稳定读取到的范围；缺失的范围会在导入弹窗中置灰。捕获配装按毕业用途归一化角色、武器、因子、祝福和召唤石等级，但保留真实的因子组合、专精路线、武器技能和角色技能。

### 导入 GBFR Logs / Relink Logs 配装

已有战斗记录时不需要启动游戏。先进入 `存档修改（离线）` → `配装预设（查看与写入）`，点击页面中的“从 GBFR Logs 批量获取队伍配装”，打开独立的 GBFR Logs 配装库。

**方法一：读取 Logs 数据库**

1. 先退出正在运行的 Logs 程序，避免数据库还没把最新记录写完。
2. 点击“选择 Logs 数据库”，选择真正的 `logs.db`，不要选择旁边的 `logs.db-wal` 或 `logs.db-shm`。
3. GBFR Logs、Endless、Relink Logs：右键其快捷方式选择“打开文件所在位置”，或进入软件解压目录；`logs.db` 通常与 Logs 程序放在同一目录。
4. SkyMeter：按 `Win + R`，输入 `%APPDATA%\app.skymeter.relink`；旧版使用 `%APPDATA%\app.astralledger.relink`。
5. 工具只读打开数据库，按角色拆分记录。卡片会明确列出已捕获和未记录的面板、因子、角色技能、召唤石、武器、武器技能、武器祝福、专精与上限突破。

**方法二：粘贴 Relink Logs 角色 JSON**

1. 在 Relink Logs 中打开一场战斗的详情，进入角色“装备”页面，点击“复制角色数据到剪贴板 (JSON)”。
2. 回到 GBFR Logs 配装库，点击“导入角色 JSON”→“读取剪贴板”→“解析并预览”。也可粘贴内容，或选择已保存的 `.json` 文件。
3. 单个角色、角色数组和包含 `playerData` 的队伍对象都可以解析；无法映射的角色或目录外数据会明确报错或跳过，不会猜测写入。

解析完成后，先点“预览实际配装”核对来源和缺失范围，再选择后续操作：

- `上传分享`：填写标题，上传脱敏后的单套配装并复制在线链接；相同标题可以重复使用。
- `导入到存档`：选择目标存档和角色配装槽位，进入分项导入确认；记录里没有的范围会置灰，不能强制覆盖。
- 部署存档前必须完全退出游戏。工具会先创建备份；外部记录不会覆盖未捕获的角色强化、命运篇章或其他存档进度。

不同 Logs 版本记录的字段并不相同。工具只采用文件中能够证明的数据，不会把“未记录”当作零值；数据库也不会被修改。
...
![战斗规则补丁页面](docs/screenshots/patch-combat-v19116.png)
...
- [在线配装图鉴](https://share.whitelinker.top) 可按角色、武器、因子和祝福搜索社区配装，预览三栏详情、复制短码、下载单套文件，并支持点赞和留言；上传内容不包含整个存档。
- 配装页分项展示存档可证明的成长、武器、因子、专精、上限突破和召唤石来源。
- 单套配装文件包含武器引用及其强化、觉醒/超凡、五个武器技能和祝福，12 个独立因子、角色技能、专精选择与等级、角色强化、四槽上限突破和全局召唤石；导入弹窗决定覆盖范围。缺失召唤石会补建，角色强化采用只增不减合并，系统开放状态与命运篇章保持目标存档原值。
- 上限突破、专精和召唤石使用审计后的独立单位：攻击与 HP 为面板整数，暴击与伤害类为百分比，昏厥原始浮点按游戏面板 ×10 显示。
